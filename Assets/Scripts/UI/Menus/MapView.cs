...
namespace UI.Menus
{
    public class MapView : UI.Menus.Common.Menu<MapView.Data>
    {
        ...
        public override async void Populate(Data data)
        {
            ...
            foreach (var nodeData in data.MapDefinition.Nodes)
            {
                float xNodeDataNormalized = Utils.Computations.Normalize(nodeData.X, 0, data.MapDefinition.XDimension, 0, 1);
                float yNodeDataNormalized = Utils.Computations.Normalize(nodeData.Y, 0, data.MapDefinition.YDimension, 0, 1);
